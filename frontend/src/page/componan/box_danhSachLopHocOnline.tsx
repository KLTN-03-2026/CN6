...
export default function Box_danhSachLopHocOnline({
  item,
  xoa,
  ...
  const CapNhat = async () => {
    const TenLopHoc = Input_Ten.current?.value.trim() || "";
    const LinkLopHoc = Input_Link.current?.value.trim() || "";
    let check = 0;
    if (TenLopHoc === "") {
    ...
    if (check === 0) {
      try {
        ...
        const api = await fetch(
          `${BACKEND_URL}/api/CapNhat-lophocon/${item._id}`,
          {
            method: "PATCH",
            headers: {
              Authorization: Token,
